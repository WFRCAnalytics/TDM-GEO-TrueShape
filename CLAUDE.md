...
- **Link Routing:** True-shape conflation is solved via a direct chain/coordinate join, not shortest-path routing: `CandidateTDMRoadLinks` is fully redissolved per `VERT_LEVEL` and split only at snapped model-node positions (`02_create_link.qmd`), then each resulting physical piece is matched to the model-link chain sharing its exact `(A,B)` snapped-node pair via a vectorized, direction-sensitive merge — no BFS or pathfinding through the physical network is involved (`03_transfer_attributes.qmd`). Every piece's real geometry is retained regardless of match success; attributes are left blank rather than fabricating straight-line geometry.
